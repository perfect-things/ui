...
- Docs site: `docs/` (Vite root) → built into `public/` (GitHub Pages)
- Library build: `svelte-package` output to `dist/` (see `npm run build:lib`)

Recommended quick commands (see `package.json` scripts):
- npm start / npm run dev — run the public/demo site (Vite, root `docs/`)
- npm run build:docs — build the docs site into `public/`
...
- Vite config is `vite.config.ts`: the dev server root is `docs/` and `publicDir` points to `../assets`. The docs build injects a version script and Plausible analytics via a transform that looks for `<!-- scripts-go-here -->` in `docs/index.html`.
...
- Docs live in `docs/` — small content edits go there. Assets used by the docs are in `assets/` (logo, index.html, base.css, etc.).
...
- Vite/docs config: `vite.config.ts` and `docs/index.html`
